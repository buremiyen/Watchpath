...
export default function DoomsdayFinale(){
 ...
 useEffect(()=>{const check=()=>{try{const p=JSON.parse(localStorage.getItem('watchpath-progress')||'{}');const v=Object.values(p);setComplete(v.length>0&&v.every(Boolean)&&v.filter(Boolean).length>=38)}catch{setComplete(false)}};check();window.addEventListener('storage',check);const t=setInterval(check,1000);return()=>{window.removeEventListener('storage',check);clearInterval(t)}},[])
 return <section className="finale">
  ...
  <article className="doomsdayGlass">
   <div className="poster"><img src="https://placehold.co/720x1080/13070b/ef4457?text=AVENGERS%0ADOOMSDAY" alt="Avengers Doomsday poster"/><div className="posterGlow"/></div>
